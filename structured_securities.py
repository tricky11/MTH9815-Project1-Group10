...
class StructuredSecurities(object):
    def __init__(self, notional):
        ...
        self._transactions = DataFrame(
            columns=['interest_due' 'interest_payment', 'interest_shortfall', 'principal_payment',
                     'notional_balance'], dtype=float)
        self._transactions['tranche_id'] = Series(dtype=str)
    ...
    def make_payments(self, cash_amount):
        ...
        if self._mode == Mode.pro_rata:
            ...
            # TODO: Put extra cash in reserve account instead of using it for next tranche?
            for tranche in self._tranches:
                available_principal_amount = min(tranche.notional * cash_amount / total_notional,
                                                 tranche.notional_balance())
                tranche.make_interest_payment(available_principal_amount)
